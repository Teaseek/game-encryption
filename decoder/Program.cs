...
else
{
  ...
  Console.Write("Enter the encrypted line: ");
...
for (var i = 0; i < lines.Length; i++)
{
  ...
  for (var ii = 0; ii < inputLine.Length; ii++)
  {
    ...
    var resultLetter = getLetter(inputLetter, keyLetter);
    ...
    stepsOutput +=
      ...
      $"({alphabetMap[inputLetter],2:0} - {alphabetMap[keyLetter],2:0} = {alphabetMap[inputLetter] - alphabetMap[keyLetter],3: 0;-0})\n";
  }
...
char getLetter(char inputLetter, char keyLetter)
{
  var isUpper = char.IsUpper(inputLetter);
  var letterNumber = alphabetMap[inputLetter] - alphabetMap[keyLetter];
  var letter =
    letterNumber > 0 ?
